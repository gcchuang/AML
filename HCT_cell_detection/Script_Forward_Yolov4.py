...
log_file = '../log/cell_selection.log'

# Open the log file in write mode
log_file_handle = open(log_file, 'a')

# Redirect TensorFlow logging to the log file
...
for slide_num in os.listdir(slides_path):
    if os.path.exists(f"./output/{slide_num}") == True:
        ...
        for patch in os.listdir(slide_path):
            ...
            crop_cells(patch.split(".")[0],original_image, pred_bboxes,classes,f"./output/{slide_num}/")
        ...
        print('cells were detected from the ROI {}, time:{:02d}m{:02d}s'.format(slide_num,int(elapsed_time // 60), int(elapsed_time % 60)))
        log_file_handle.flush()
...
# cv2.imwrite(f"./output/{slide_num}_{patch_number}/{slide_num}_{patch_number}.png", result)
# # counting the elapsed time
# end_time = time.time()
# elapsed_time = end_time - start_time
